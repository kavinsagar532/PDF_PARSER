...
class FileOperation:
    """Immutable record of a file operation for diagnostics."""
...
class SupportsFileIO(Protocol):
    """Protocol describing file I/O capability."""

    def read_jsonl(self, file_path: str) -> Generator[Dict[str, Any], None, None]:
        """Read JSONL entries from a file path."""
        ...

    def write_jsonl(
        self, file_path: str, data: Iterable[Dict[str, Any]], mode: str = "w"
    ) -> int:
        """Write JSONL entries to a file path."""
        ...


class FileOperationFactory(ABC):
    """Abstract factory for creating file operation records."""
...
class DefaultFileOperationFactory(FileOperationFactory):
    """Concrete factory for file operation records."""
